...
/* Forward */
static void dumptoken(DAP2lexstate* lexstate);
static void dapaddyytext(DAP2lexstate* lex, int c);
#ifndef DAP2ENCODE
...
int
dap2lex(YYSTYPE* lvalp, DAP2parsestate* state)
{
    DAP2lexstate* lexstate = state->lexstate;
    int token;
    int c;
    unsigned int i;
    char* p;
...
static void
dapaddyytext(DAP2lexstate* lex, int c)
{
...
static void
dumptoken(DAP2lexstate* lexstate)
{
...
void
dap2setwordchars(DAP2lexstate* lexstate, int kind)
{
    switch (kind) {
    case 0:
...
void
dap2lexinit(char* input, DAP2lexstate** lexstatep)
{
    DAP2lexstate* lexstate;
    if(lexstatep == NULL) return; /* no point in building it */
    lexstate = (DAP2lexstate*)malloc(sizeof(DAP2lexstate));
    *lexstatep = lexstate;
    if(lexstate == NULL) return;
    memset((void*)lexstate,0,sizeof(DAP2lexstate));
    lexstate->input = strdup(input);
    lexstate->next = lexstate->input;
    lexstate->yytext = ocbytesnew();
    lexstate->reclaim = oclistnew();
    dap2setwordchars(lexstate,0); /* Assume DDS */
}

void
dap2lexcleanup(DAP2lexstate** lexstatep)
{
    DAP2lexstate* lexstate = *lexstatep;
    if(lexstate == NULL) return;
...
char*
dap2decode(DAP2lexstate* lexstate, char* name)
{
    char* decoded = NULL;
