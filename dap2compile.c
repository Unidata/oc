...
OCerror
dap2_compile(OCstate* state, OCnode* xroot)
{
...
occompile1(OCstate* state, OCnode* xnode, XXDR* xxdrs, OCdata** datap)
{
	...
	for(nelements=0;;nelements++) {
	    ...
	    if(!xxdr_opaque(xxdrs,tmp,(size_t)sizeof(tmp)))
		{ocstat = OC_EXDR; goto fail;}
...
occompileatomic(OCstate* state, OCdata* data, XXDR* xxdrs)
{
    ...
    if(!scalar) {
        ...
        if(xnode->basetype != OC_String && xnode->basetype != OC_URL) {
    ...
    data->xdrsize = ocxdrsize(xnode->basetype,scalar);

    switch (xnode->basetype) {
	...
	data->strings = (size_t*)malloc((size_t)(sizeof(size_t)*data->nstrings));
	/* We need to walk each string, get size, then skip */
	    ...
	    len = RNDUP((size_t)len);
	    /* Skip the data */
	    xxdr_skip(xxdrs,(size_t)len);
        }
        break;

    default:
	OCPANIC1("unexpected basetype: %d",xnode->basetype);
...
void
dap2_data_free(OCstate* state, OCdata* data)
{
    if(data == NULL)
	return;
...
static size_t
ocxdrsize(OCtype basetype, int isscalar)
{
    switch (basetype) {
    case OC_Char:
    case OC_Int8:
...
ocerrorstring(XXDR* xdrs)
{
    ...
    size_t avail = xxdr_getavail(xdrs);
    char* data = (char*)malloc((size_t)avail);
    if(!xxdr_setpos(xdrs,(size_t)0)) return 0;
