...
pkgdatadir = $(datadir)/oc
pkgincludedir = $(includedir)/oc
pkglibdir = $(libdir)/oc
pkglibexecdir = $(libexecdir)/oc
...
DIST_COMMON = $(am__configure_deps) $(pkginclude_HEADERS) \
	...
	$(top_srcdir)/conf/mkinstalldirs $(top_srcdir)/configure \
	COPYING ChangeLog INSTALL conf/compile conf/config.guess \
	conf/config.sub conf/depcomp conf/install-sh conf/ltmain.sh \
	conf/missing conf/mkinstalldirs
ACLOCAL_M4 = $(top_srcdir)/aclocal.m4
am__aclocal_m4_deps = $(top_srcdir)/conf/libtool.m4 \
	$(top_srcdir)/conf/ltoptions.m4 $(top_srcdir)/conf/ltsugar.m4 \
	$(top_srcdir)/conf/ltversion.m4 \
	$(top_srcdir)/conf/lt~obsolete.m4 $(top_srcdir)/configure.ac
...
ACLOCAL = ${SHELL} /home/dmh/git/oc/conf/missing --run aclocal-1.12
ALLOCA = 
AMTAR = $${TAR-tar}
AR = ar
AUTOCONF = ${SHELL} /home/dmh/git/oc/conf/missing --run autoconf
AUTOHEADER = ${SHELL} /home/dmh/git/oc/conf/missing --run autoheader
AUTOMAKE = ${SHELL} /home/dmh/git/oc/conf/missing --run automake-1.12
AWK = gawk
CC = gcc
...
LN_S = ln -s
LTLIBOBJS = 
MAKEINFO = ${SHELL} /home/dmh/git/oc/conf/missing --run makeinfo
MANIFEST_TOOL = :
MKDIR_P = /bin/mkdir -p
NM = /usr/bin/nm -B
NMEDIT = 
...
OTOOL64 = 
PACKAGE = oc
PACKAGE_BUGREPORT = support@opendap.org
PACKAGE_NAME = oc
PACKAGE_STRING = oc 2.0
PACKAGE_TARNAME = oc
PACKAGE_URL = 
PACKAGE_VERSION = 2.0
PATH_SEPARATOR = :
...
SHELL = /bin/sh
STRIP = strip
VALGRIND = valgrind --logfile=memcheck
VERSION = 2.0
YACC = bison -y
YFLAGS = 
abs_builddir = /home/dmh/git/oc
abs_srcdir = /home/dmh/git/oc
abs_top_builddir = /home/dmh/git/oc
abs_top_srcdir = /home/dmh/git/oc
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_DUMPBIN = 
am__include = include
...
install_sh = ${SHELL} /home/dmh/git/oc/conf/install-sh
...
liboc_la_LDFLAGS = -version-info $(LIBOC_VERSION) -L/usr/local/lib -lcurl 
...
oc-config: $(top_builddir)/config.status $(srcdir)/oc-config.in
...
liboc.la: $(liboc_la_OBJECTS) $(liboc_la_DEPENDENCIES) $(EXTRA_liboc_la_DEPENDENCIES) 
	$(liboc_la_LINK) -rpath $(libdir) $(liboc_la_OBJECTS) $(liboc_la_LIBADD) $(LIBS)
...
ocprint$(EXEEXT): $(ocprint_OBJECTS) $(ocprint_DEPENDENCIES) $(EXTRA_ocprint_DEPENDENCIES) 
	@rm -f ocprint$(EXEEXT)
	$(LINK) $(ocprint_OBJECTS) $(ocprint_LDADD) $(LIBS)
...
include ./$(DEPDIR)/ocdump.Plo
include ./$(DEPDIR)/ochttp.Plo
include ./$(DEPDIR)/ocinternal.Plo
include ./$(DEPDIR)/oclist.Plo
include ./$(DEPDIR)/oclog.Plo
include ./$(DEPDIR)/ocnode.Plo
include ./$(DEPDIR)/ocprint.Po
include ./$(DEPDIR)/ocrc.Plo
include ./$(DEPDIR)/ocread.Plo
...
installdirs-am:
	for dir in "$(DESTDIR)$(libdir)" "$(DESTDIR)$(bindir)" "$(DESTDIR)$(bindir)" "$(DESTDIR)$(pkgincludedir)"; do \
...
srpm: dist
	rpmbuild -ts $(RPM_OPTIONS) oc-2.0.tar.gz

rpm: dist
	rpmbuild -tb $(RPM_OPTIONS) oc-2.0.tar.gz

clean-pkg:
	-rm -rf mac_osx oc-2.0.pkg
...
pkg-main: pkg-build
	...
	cat ./OSX_Resources/Info.plist.proto | \
	   sed -e "s^_PREFIX_^$(prefix)^g" \
	       -e "s^_FULL_VERSION_^2.0^g" \
	       -e "s^_MAJOR_VERSION_^@PACKAGE_MAJOR_VERSION@^g" \
	       -e "s^_MINOR_VERSION_^@PACKAGE_MINOR_VERSION@^g" \
	       > foo
	mv foo ./OSX_Resources/Info.plist
	${PACKAGEMAKER} --root mac_osx --id org.opendap.oc \
	    --title "oc 2.0" --version 2.0 \
	    --out oc-2.0.pkg --resources OSX_Resources

pkg-dmg: pkg-main
	-rm -rf oc-2.0
	-rm -rf oc-2.0.dmg
	mkdir oc-2.0
	cp -r oc-2.0.pkg oc-2.0
	cp README RELEASE_NOTES oc-2.0
	dropdmg -i --sanitize-for-servers --format zlib oc-2.0
	-rm -rf oc-2.0
